...
std::string MethodMoveTransform::rewriteMethodInHeader(CXXMethodDecl *M)
{
	...
	while (PI != PE) {
		...
		//auto PIE = (*PI)->getSourceRange().getEnd();
		//llvm::errs() << captureSourceText(PIB, PIE, true) << "\n";
		llvm::errs() << (*PI)->getName() << "\n";
	...
	if( MTSI)
	{
		...
		MTLE = sema->getPreprocessor().getLocForEndOfToken(MTL.getEndLoc());
	}
	else
		MTLE = M->getBody()->getLocStart();
	...
	//if ( M->getAttr<OverrideAttr>() ) {
	//	insert(replaceRange, " override");
	//}
